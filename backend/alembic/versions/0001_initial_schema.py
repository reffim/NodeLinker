...
# Define enum types once using postgresql.ENUM with create_type=False
# The actual CREATE TYPE is done via op.execute below (idempotent via DO block)
user_role = postgresql.ENUM("admin", "operator", "viewer", name="user_role", create_type=False)
node_status = postgresql.ENUM("online", "offline", "unreachable", "unknown", name="node_status", create_type=False)
playbook_source = postgresql.ENUM("local", "git", name="playbook_source", create_type=False)
job_status = postgresql.ENUM("pending", "running", "success", "failed", "cancelled", name="job_status", create_type=False)
job_node_status = postgresql.ENUM("pending", "running", "success", "failed", name="job_node_status", create_type=False)


def upgrade() -> None:
    # Create enums explicitly first (idempotent via DO block)
    op.execute("""
        DO $$ BEGIN
    ...
    op.execute("""
        DO $$ BEGIN
            CREATE TYPE node_status AS ENUM ('online', 'offline', 'unreachable', 'unknown');
        EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            CREATE TYPE playbook_source AS ENUM ('local', 'git');
        EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    """)
    op.execute("""
        DO $$ BEGIN
    ...
    op.execute("""
        DO $$ BEGIN
            CREATE TYPE job_node_status AS ENUM ('pending', 'running', 'success', 'failed');
        EXCEPTION WHEN duplicate_object THEN NULL; END $$;
    """)
    ...
    op.create_table(
        ...
        sa.Column("source_type", playbook_source, nullable=False, server_default="local"),
    ...
    op.create_table(
        ...
        sa.Column("status", job_node_status, nullable=False, server_default="pending"),
